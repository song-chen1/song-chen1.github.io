...
$(function () { //获取处理友链数据
    $.getJSON("../json_data/friend.json", function (data) {
        ...
        $('.links-content').append("<div class='friend-title-item'><br>BLOGROLL<br><br><hr></div>");
        $.each(data, function (i, e) {
            var html = "<div class=\"friend-card-item\">";
            if (e.src == undefined) {
                html += "    <img class=\"ava\" src=\"/img/links/nopic.jpg\" title=\"Image link not available, default image used\">";
            } else {
                html += "    <img class=\"ava\" src=\"" + e.src + "\">";
            }
            html +=
                "<div class='text-desc' title=\""+e.desc+"\">    URL：<a href=\"" + e.url + "\" target=\"_blank\">" + e.name + "</a>" +
                "    <br>Date：" + e.date +
                "<br>intro：" + e.desc + "</div>" +
                "    </div>";
